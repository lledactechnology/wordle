...
function resetToMenu(){if(_reconnectTimer){clearTimeout(_reconnectTimer);_reconnectTimer=null;}_reconnecting=false;_reconnectAttempts=0;if(ws){try{ws.close();}catch(e){}}sessionStorage.removeItem('wordle_roomId');sessionStorage.removeItem('wordle_playerName');sessionStorage.removeItem('wordle_playerToken');sessionStorage.removeItem('wordle_roundComplete');gameState='menu';isSoloMode=false;isHost=false;roomId=null;playerId=null;soloTargetWord='';roundSolved=false;myGameComplete=false;currentRow=0;currentGuess=[];leaderboardData=[];spectateData={};resetBoard();showScreen('menu');}
